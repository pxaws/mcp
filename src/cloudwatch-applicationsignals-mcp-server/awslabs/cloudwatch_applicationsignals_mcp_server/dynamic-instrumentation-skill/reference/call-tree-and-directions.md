...
Add a "Call Tree" section to your debugging report:
...
Direction choices are defined in SKILL.md under "Direction Choices — Where to Go Next". This reference file focuses on **visual call-tree patterns** for communicating investigation progress.

For a quick reminder, the directions are:
- **Upstream** — suspicious inputs, find who passed them
- **Downstream** — correct inputs but wrong output, go inside
- **Line-level** — need intermediate state within the function
- **Multi-snapshot** — intermittent issue, compare good vs bad cases
